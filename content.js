...
(function() {
    ...
    // ==================== Jellyfin 检测 ====================
    // 在所有页面加载，但只在 Jellyfin 页面激活
    function isJellyfinPage() {
        if (document.getElementById('jellyfin')) return true;
        if (document.querySelector('meta[name="generator"][content*="Jellyfin"]')) return true;
        if (document.querySelector('script[src*="jellyfin"]')) return true;
        return false;
    }
    ...
    function initExtension() {
        ...
        console.log('[PP] v10 loaded (universal)');
        ...
        function getAuth() {
            try {
                ...
                console.warn('[PP] getAuth failed:', e);
            }
            return null;
        }
        ...
        function inject() {
            try {
                ...
                console.warn('[PP] inject() error:', e);
            }
        }
        ...
        try {
            ...
            console.warn('[PP] Failed to observe body:', e);
        }

        try {
            inject();
        } catch (e) {
            console.warn('[PP] Initial inject failed:', e);
        }
    ...
    // ==================== 启动 ====================
    if (isJellyfinPage()) {
        console.log('[PP] Jellyfin detected, loading extension...');
        initExtension();
    } else {
        // SPA 首屏可能未渲染完，用 MutationObserver 等待 Jellyfin 出现
        var found = false;
        var waitObserver = new MutationObserver(function() {
            ...
            if (isJellyfinPage()) {
                ...
                console.log('[PP] Jellyfin detected (lazy), loading extension...');
                initExtension();
            }
        });
        waitObserver.observe(document.documentElement, { childList: true, subtree: true });
        // 5 秒后仍未检测到则放弃
        setTimeout(function() { if (!found) { found = true; waitObserver.disconnect(); } }, 5000);
    }
})();
